...
if __name__ == "__main__":
    ...
    for node in nodes:
        for chromo_id in per_chromo_timestamps[node]:
            per_chromo_sorting_time[chromo_id] = per_chromo_timestamps[node][chromo_id][-1] - per_chromo_timestamps[node][chromo_id][0]

    with open("sorting_time_N_nodes.txt", "w") as f:
        for chromo in CHROMOS:
            f.write(chromo + "\t" + str(per_chromo_sorting_time[chromo]) + "\n")
