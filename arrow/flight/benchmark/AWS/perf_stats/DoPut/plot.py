...
if __name__ == "__main__":
    for i in range(8):
        perf_stats = pd.read_csv("perf_stats_1_{}.csv".format(i + 1),
                                 sep="\t",
                                 names=col_names)
        ...
        ax.set_xticklabels(speeds["Number of records per stream"].unique(),
                           rotation=30)
        ax.title.set_text("(DoPut, number of streams: {})".format(i + 1))
        ax.set_xlabel("Number of records per stream")
        ax.set_ylabel("Speed (MB/s)")
        plt.savefig("perf_stats_1_{}_DoPut.png".format(i + 1), dpi=300)
